...
if (!isMobile) {
  document.getElementById('notSupportedMsg').style.display = 'block';
} else if (isIOS) {
  // iOS fallback AR
  loadIOSAR();
} else {
...
function loadIOSAR() {
  // This is a simple fallback AR using device orientation for iOS
  alert("iOS AR mode activated! Tap to place a model.");

  let scene, camera, renderer, model, modelDropdown;
  ...
  document.getElementById('arButton').addEventListener('click', () => {
    ...
    loader.load(modelPath, (gltf) => {
      ...
      model.position.set(0, 0, -2); // In front of the user
      scene.add(model);
    });
  });
...
/* Android (WebXR) code below (same as the working code you provided) */
let scene, camera, renderer, xrSession, xrReferenceSpace, xrHitTestSource;
let infoDiv, warningDiv, loadingDiv, exitButton, modelDropdown;
